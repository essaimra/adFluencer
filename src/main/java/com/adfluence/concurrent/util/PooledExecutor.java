...
public class PooledExecutor extends ThreadFactoryUser implements Executor {
     ...
     **/
    public static final long DEFAULT_KEEPALIVETIME = 60 * 100000;
     ...
     **/
    protected final Channel boundBuffer;
     ...
     * when holding lock.
     **/
    protected void addThread(Runnable command, int i) {
        Worker worker = new Worker(command);
        Thread thread = getThreadFactory().newThread(worker, i);
    ...
    protected synchronized void workerDone(Worker w) {
        ...
        if (poolSize == 0 || poolSize < minimumPoolSize) {
            try {
                Runnable r = (Runnable) (boundBuffer.poll(0));
    ...
    protected Runnable getTask() throws InterruptedException {
        ...
        if (waitTime >= 0) {
            return (Runnable) (boundBuffer.poll(waitTime));
        }
        else {
            return (Runnable) (boundBuffer.take());
        }
    }
    ...
    protected class DiscardOldestWhenBlocked implements BlockedExecutionHandler {
        public boolean blockedAction(Runnable command) throws InterruptedException {
            boundBuffer.poll(0);
            if (!boundBuffer.offer(command, 0))
                command.run();
            return true;
        }
    }
